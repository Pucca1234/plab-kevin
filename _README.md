...
## 문서 안내
- 실행/운영 개요: 이 `README.md`
- 현재 문서 허브: `docs/README.md`
- 문서 작성 규칙: `docs/DOCUMENTATION_RULES.md`
- 마스터 PRD: `docs/MASTER_PRD.md`
- 기능별 PRD: `docs/feature-prds/*`
- 작업 TODO: `docs/todo/ACTIVE_TODO.md`, `docs/todo/BACKLOG_TODO.md`

## 문서 역할 원칙
- `README.md`는 실행 방법, 환경변수, 운영 절차, 문서 진입점을 설명합니다.
- `PRD.md`는 레거시 통합 기준 문서로 유지하되, 신규 상세 정책은 `docs/MASTER_PRD.md`와 기능별 PRD에 먼저 기록합니다.
- 후속 작업과 파생 작업은 채팅이 아니라 `docs/todo/*`에 남깁니다.
...
## 2026-04-25 기간 범위/증감 표시 보정
- 기간 범위 `전체` 조회 보정:
  - 기존에는 `전체` 선택 시에도 조회일 기준 미래 기간이 잘려, 데이터가 있는 미래 일정이 결과/기간 필터에서 제외됨
  - `/api/weeks`에 `includeFuture` 옵션을 추가하고, `기간 범위=전체`일 때는 미래 기간도 포함한 전체 기간 목록을 반환하도록 조정
  - `week` 단위도 `전체` 조회 시에는 source 기준 전체 기간을 읽어 미래 주차가 있으면 함께 노출
- 기간 필터 선택 상태 보정:
  - 기존에는 `기간단위=주`, `기간 범위=24주`처럼 범위를 바꿔도 이전 8주 선택 상태가 남아 실제 기간 필터가 좁게 유지되는 문제가 있었음
  - 기간 범위 변경 시 기존 기간 필터 선택값을 초기화하고, 새 범위 기준 옵션 전체가 다시 선택되도록 수정
  - 예: `24주` 선택 후 기간 필터를 열면 최근 24주가 선택된 상태로 표시
- 결과 테이블 증감 표시 단위 변경:
  - 퍼센트 지표는 기존대로 이전 대비 `%p` 유지
  - 일반 수치 지표는 절대값 차이 대신 이전값 대비 증감률 `%`로 표시하도록 변경
  - 동일 규칙을 결과 테이블 외 hover/info/chart 보조 표기에도 맞춤

## 2026-04-29 툴팁 레이어 / 결과 이후 자동 재조회 보정
- 추이 그래프 툴팁 가림 보정:
  - 결과 테이블의 스파크라인 hover 툴팁이 상단 헤더 레이어에 가려져 일부 내용이 보이지 않는 문제를 수정
  - 스파크라인 툴팁과 지표 설명 툴팁의 레이어 우선순위를 높여 sticky header/control 영역 위에 정상 노출되도록 조정
- 조회 방식 개선:
  - 최초 진입은 기존처럼 사용자가 `조회` 버튼을 눌러 결과를 띄우는 방식을 유지
  - 한 번 결과가 나온 이후에는 기간단위, 기간범위, 측정단위, 동적 필터, 선택 지표 변경 시 결과가 자동으로 다시 조회되도록 변경
  - 과도한 연속 요청을 줄이기 위해 짧은 debounce를 적용하고, 필터 옵션 재계산 중에는 자동 재조회를 잠시 보류
  - 템플릿 적용, 기본값 복원, 수동 조회 직전에는 대기 중인 자동 재조회 타이머를 정리해 중복 요청을 방지

## 2026-04-29 후속 보정: 스파크라인 포털 / 멀티셀렉트 즉시 반영
- 스파크라인 툴팁 표시 보정 2차:
  - 1차 보정 후에도 툴팁이 `table-scroll`/`data-grid` 영역 문맥에 남아 상단 영역에 가려지는 문제가 지속됨
  - 스파크라인 툴팁을 셀 내부 `absolute` 렌더링에서 `document.body` portal + `position: fixed` 방식으로 변경
  - 이로써 테이블 스크롤 컨텍스트와 분리해 헤더/테이블 sticky 영역에 덜 종속되도록 조정
- 동적 필터 즉시 반영 보정 2차:
  - 1차 자동 재조회 보정 후에도 다중선택 드롭다운은 내부 `draft` 상태만 바뀌고, 메뉴를 닫기 전에는 실제 `onChange`가 호출되지 않아 결과가 그대로 남는 문제가 있었음
  - `MultiSelectDropdown`에서 체크/해제 시점에 즉시 `onChange`를 호출하도록 수정
  - 이에 따라 결과가 이미 표시된 상태에서는 필터 선택/해제가 실제 조회 조건에 바로 반영됨

## 2026-04-29 후속 보정 3차: 기간 필터 선택값을 결과 컬럼 축에 반영
- 증상:
  - 기간범위가 `최근 8주`인 상태에서 기간 필터(`주`)에서 4개 주만 선택해도 결과 테이블 컬럼은 여전히 8주 전체가 노출됨
- 원인:
  - 자동 재조회는 동작했지만, `runSearch()`가 `/api/weeks`에서 받은 기간범위 기준 전체 목록을 그대로 `weeks` 축으로 사용하고 있었음
  - 즉, 기간 필터 선택값은 heatmap 조건 일부에는 반영돼도 결과 컬럼 축 자체는 줄어들지 않았음
- 조치:
...
## 2026-05-06 드릴다운 중 필터/데이터 재로딩 경합 보정
- 증상:
  - 측정단위를 여러 단계로 드릴다운한 뒤 필터 로딩과 데이터 재조회가 엇갈리면, 최종 드릴다운 결과가 잠깐 보였다가 상위 측정단위 결과로 되돌아가는 문제가 있었음
  - 예: `요일그룹 -> 주말 -> 요일` 드릴다운 후 `요일` 결과가 나온 뒤 다시 `요일그룹` 결과로 복귀
- 원인:
  - 결과가 이미 표시된 상태에서 자동 재조회가 실행될 때 공통 `handleSearch()`가 항상 루트 조회 기준(`drilldownParent=null`)으로 재검색을 수행했음
  - 그 결과, 드릴다운 직후 측정단위 변경으로 시작된 필터 재로딩이 끝난 다음 자동 재조회가 늦게 실행되면 최신 드릴다운 결과를 상위 단계 응답이 덮어쓸 수 있었음
- 조치:
  - 자동 재조회와 수동 조회 모두 현재 `drilldownParent`를 유지한 상태로 `runSearch()`를 호출하도록 수정
  - 드릴다운 상태에서는 최신 측정단위/부모 엔티티 기준으로 마지막 breadcrumb 항목을 다시 구성해, 재조회 후에도 현재 경로가 유지되도록 보정
- 기대 결과:
  - 2depth 이상 드릴다운 상태에서도 필터 옵션 재계산과 데이터 재조회가 이어져도 마지막 사용자가 선택한 드릴다운 결과가 유지됨
  - 자동 재조회 UX는 유지하면서도 상위 단위 결과로 되돌아가는 현상을 방지
- 검증:
  - `npm run build`
  - `git diff --check`

## 2026-05-06 후속 보정: 필터 로딩 완료 시 불필요한 자동 재조회 방지
- 증상:
  - 이전 보정 후 상위 결과로 되돌아가는 문제는 해결됐지만, 결과가 이미 맞게 표시된 상태에서도 필터 로딩이 뒤늦게 끝나면 화면이 한 번 더 새로고침되는 듯한 재조회가 발생했음
- 원인:
  - 자동 재조회 effect가 사용자 입력뿐 아니라 `isLoadingFilter` 종료 시점에도 다시 평가되면서, 마지막 조회 시그니처가 같거나 결과 변화가 없더라도 debounce 후 `handleSearch()`를 다시 실행할 수 있었음
- 조치:
  - 자동 재조회는 사용자 입력으로 `requestAutoRefresh()`가 호출돼 토큰이 증가한 경우에만 1회 실행되도록 변경
  - 필터 로딩 완료 자체는 재조회 트리거로 쓰지 않고, 필터 UI 상태만 마저 갱신하도록 보정
- 기대 결과:
  - 필터 옵션이 늦게 도착해도 데이터 결과가 바뀌지 않으면 화면이 부자연스럽게 다시 로딩되지 않음
  - 실제 사용자 변경이 있을 때만 자동 재조회가 1회 실행되고, 이후 비동기 필터 로딩 완료는 결과 테이블에 영향을 주지 않음
- 검증:
  - `npm run build`
  - `git diff --check`

## 2026-05-06 기간 컬럼 드릴다운 / 정렬 아이콘 분리
- 목표:
  - 기존 엔티티 드릴다운과 별도로, 결과 테이블의 기간 컬럼도 클릭해 하위 기간단위로 내려갈 수 있도록 확장
  - 예: `기간단위=주` 결과에서 `26.04.06 - 04.12` 컬럼 클릭 후 `일` 선택 시, 해당 주 범위 내부의 일 단위 결과를 표시
- 구현 방식:
  - 기간 컬럼 본문 클릭은 `기간 드릴다운 메뉴`를 열고, 현재 표시 중인 기간단위보다 더 세분화된 하위 단위만 노출
  - 결과 조회는 전체 기간단위를 바꾸는 것이 아니라, 클릭한 상위 기간의 날짜 범위 내부로 들어가는 방식으로 처리
  - `/api/weeks?includeStartDate=1` 응답의 `startDate`를 사용해 상위 기간 범위 안에 포함되는 하위 기간 목록만 잘라서 heatmap/필터 조회에 전달
- UI 정책:
  - 기간 텍스트 영역은 드릴다운 클릭 영역
  - 기간명 오른쪽의 위/아래 정렬 아이콘은 기존 기간별 정렬 전용 버튼으로 분리
  - 정렬과 드릴다운이 같은 클릭 영역을 공유하지 않도록 역할을 분리
- 상태/복귀:
  - 기간 드릴다운은 별도 breadcrumb로 현재 경로를 표시
  - 예: `주(26.04.06 - 04.12) > 일`
  - `기간 드릴다운 해제` 버튼으로 루트 기간 조회로 복귀 가능
- 기대 결과:
  - 엔티티 드릴다운과 기간 드릴다운이 동시에 있어도, 현재 엔티티 컨텍스트와 기간 컨텍스트를 함께 유지한 결과를 제공
  - 기간 컬럼 헤더에서 드릴다운과 정렬이 서로 충돌하지 않음
- 검증:
  - `npm run build`
  - `git diff --check`

## 2026-05-07 후속 보정: 기간 컬럼 정렬 아이콘 상시 노출
- 증상:
  - 기간 드릴다운/정렬 분리 반영 후, 결과 테이블의 기간 컬럼 오른쪽에 있어야 할 정렬 아이콘이 기본 상태에서 보이지 않았음
- 원인:
  - 엔티티 테이블 헤더에서 정렬 버튼이 활성 정렬 상태일 때만 아이콘을 렌더링하고 있었음
  - 이 때문에 비정렬 기본 상태에서는 정렬 버튼 클릭 영역은 있어도 사용자에게 아이콘이 보이지 않았음
- 조치:
  - 기간 컬럼 정렬 버튼은 비활성 상태에서도 위/아래 정렬 아이콘을 항상 렌더링하도록 수정
  - 현재 정렬 방향만 opacity 차이로 표시해, 클릭 전에도 정렬 기능이 있는 것을 인지할 수 있게 보정
- 기대 결과:
  - 모든 기간 컬럼 헤더에서 드릴다운 텍스트 오른쪽에 정렬 아이콘이 항상 보임
  - 사용자는 드릴다운 클릭 영역과 정렬 클릭 영역을 시각적으로 명확히 구분할 수 있음
- 검증:
  - `npm run build`
  - `git diff --check`

## 2026-05-07 후속 보정 2차: 필터 단독 선택 UX / 일 전체 기간 필터 / 분기 기간 드릴다운
- `지정된 값만 보기` 버튼 가시성 보정:
  - 필터 드롭다운 hover 시 노출되는 `지정된 값만 보기` 버튼 문구가 `지정된 값만...`으로 잘려 보이던 문제를 수정
  - 버튼 영역 폭을 넓히고, 액션 버튼 자체에는 말줄임을 적용하지 않도록 보정
- `지정된 값만 보기` 상호작용 보정:
  - 기존에는 해당 버튼 클릭 시 값 단독 선택과 조회는 되지만 드롭다운이 즉시 닫혀 추가 선택 흐름이 끊겼음
  - 이제는 해당 값만 단독 선택해 즉시 조회하면서도 드롭다운은 열린 상태를 유지하도록 수정
- `기간단위=일`, `기간범위=전체` 필터 노출 보정:
  - 전체 일자 범위를 조회할 때 연/분기/월/주 기간 필터가 비정상적으로 사라지던 문제를 수정
  - 전체 범위에서는 과도한 개별 period 파라미터를 요청에 붙이지 않고, 서버가 전체 범위를 기준으로 기간 필터 축을 계산하도록 조정
- `기간단위=분기` 기간 드릴다운 보정:
  - 결과 테이블에서 분기 컬럼 클릭 후 하위 기간단위를 선택해도 반응이 없던 문제를 수정
  - 표시용 분기 라벨(`26년 2분기`)과 실제 period key(`26.2`)를 함께 매칭해, 클릭한 분기 컬럼이 올바른 기간 범위로 해석되도록 보정
- 검증:
  - `npm run build`
  - `git diff --check`

## 2026-05-10 후속 보정: 필터 자동조회 / 전체 기간 필터 컬럼 축 정합성
- 필터 선택 후 자동조회 복구:
  - 필터 선택 직후 예약된 자동조회가 필터 옵션 로딩으로 잠시 보류된 뒤, 이미 처리된 토큰으로 간주되어 실제 조회가 실행되지 않는 문제가 있었음
  - 자동조회 토큰을 debounce 타이머 예약 시점이 아니라 실제 `handleSearch()` 실행 직전에 처리 완료로 표시하도록 변경
  - 결과가 이미 표시된 상태에서는 필터 선택/해제 후 필터 로딩이 끝나면 자동조회가 정상 실행됨
- `기간범위=전체` 기간 필터 결과 축 보정:
  - 기존에는 결과 기간단위와 같은 필터만 결과 컬럼 축에 반영되어, `기간단위=월` + `기간범위=전체` + `분기=25.2` 선택 시 전체 월이 그대로 노출될 수 있었음
  - 기간 필터 전체(`연/분기/월/주/일`)를 날짜 범위로 해석해 현재 결과 기간 엔트리의 `startDate`와 교집합 처리하도록 수정
  - 예: 월 단위 전체 조회에서 분기 `25.2`만 선택하면 25년 2분기에 포함되는 월 컬럼만 결과에 표시
- 검증:
  - `npm run build`
  - `git diff --check`
...
## 2026-05-12 후속 보정: 데이터 소스 표기 / 필터 비즈니스 로직 정리
- 상단 데이터 소스 표기 정정:
  - 운영 대시보드 상단 헤더의 `데이터 소스` 문구를 `Supabase`에서 `BigQuery`로 수정
  - 현재 `plab-kevin`의 분석 source of truth는 BigQuery이며, Supabase는 인증/템플릿/사용자 설정 등 앱 운영 데이터에 사용
- 필터 기능의 비즈니스 의미 정리:
  - 필터는 단순 UI가 아니라 `언제(기간단위/기간범위)`, `어떤 레벨로(측정단위)`, `어떤 대상만 볼지(기간/엔티티 필터)`를 단계적으로 좁혀가는 탐색형 분석 구조
  - 기간단위와 기간범위는 분석의 시간 축을 정의
  - 측정단위는 결과를 어떤 집계 레벨로 비교할지 정의
    - 예: `지역그룹 -> 지역 -> 구장 -> 면`
  - 기간/엔티티 필터는 현재 축 안에서 실제로 남길 대상을 선택하는 역할
  - 각 필터는 서로 연동되며, 한 조건을 바꾸면 다른 필터 후보도 가능한 값만 남도록 재계산
  - 한 번 결과가 표시된 이후에는 필터 변경이 자동 재조회로 이어져, 사용자가 조건을 조정하면서 바로 비교 가능
  - 기간 필터는 heatmap 조건뿐 아니라 결과 테이블의 기간 컬럼 축에도 동일하게 반영됨
    - 예: `최근 8주` 중 4주만 선택하면 결과 컬럼도 4개만 남음
  - 드릴다운 상태에서도 필터 컨텍스트는 유지되며, 하위 레벨 분석 중에도 같은 조건을 이어서 조정 가능

## 2026-05-14 후속 보정: 오늘 포함 미완료 기간 처리
- 미완료 기간 판정 기준 보정:
  - 기존에는 아직 오지 않은 `내일` 이후 기간만 미완료로 취급해 흐리게 표시하고 추이 계산에서 제외했음
  - 이제는 `오늘`이 포함된 기간부터 미완료로 간주해 동일하게 흐리게 표시하고 추이 그래프 계산에서 제외함
- 운영 의도:
  - 오늘 데이터는 일 종료 전이거나 BigQuery 집계 반영 전일 수 있어 확정 수치처럼 보이면 혼선이 생길 수 있음
  - 사용자가 오늘 데이터를 완료된 값으로 해석하지 않도록 시각적 표시와 추이 계산 기준을 일치시킴
- 기대 결과:
  - 일 단위에서는 `오늘`, 주/월/분기/연 단위에서는 현재 진행 중인 기간이 모두 미완료 상태로 처리됨
  - 미완료 기간은 결과 테이블에서 흐리게 보이고, 스파크라인/추세선 계산에서도 제외됨

## 2026-05-19 후속 보정: 연동 필터 후보 계산 / 템플릿 탭 추가 위치
- 연동 필터 후보 계산 보정:
  - 증상: `측정단위=타임` 선택 후 `요일그룹` 필터를 설정하면 선택 상태가 초기화되거나 전체 선택처럼 보일 수 있었음
  - 원인: 필터 후보를 다시 계산할 때, 현재 후보를 계산 중인 필터 축의 선택값까지 active 조건에 포함해 자기 자신을 다시 좁히고 있었음
  - 조치: `/api/filter-options-batch`에서 각 필터 축의 후보를 계산할 때는 해당 축의 active filter를 제외하고, 다른 필터 조건만 반영하도록 변경
  - 기대 결과: `요일그룹`, `지역그룹`, `타임` 등 어떤 필터든 자기 선택값 때문에 후보가 과도하게 줄어들거나 선택 상태가 초기화되는 문제를 방지
- 템플릿 탭 추가 위치 보정:
  - 증상: `+` 버튼으로 새 템플릿을 만들면 탭이 제일 우측이 아니라 두 번째 위치에 추가됨
  - 원인: 템플릿 목록 API가 `updated_at desc` 중심으로 정렬해 신규/수정 항목이 앞쪽으로 재배치됨
  - 조치: 템플릿 목록 기본 정렬을 `created_at asc`로 변경하고, 새 템플릿 생성 직후에는 클라이언트 탭 목록의 마지막에 append
  - 기대 결과: 새 템플릿은 항상 가장 오른쪽 탭으로 추가됨
- 검증:
  - `npm run build`
  - `git diff --check`
