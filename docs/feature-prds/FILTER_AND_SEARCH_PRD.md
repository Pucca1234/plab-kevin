...
### 예정된 재설계 초점
- 현재 우선 과제는 동적 필터를 "양방향 재계산"이 아니라 "앞에서 뒤로 좁혀가는 단방향 흐름"으로 재구성하는 것입니다.
- 각 필터는 처음 열렸을 때 본 후보 리스트를 다시 열어도 그대로 보여줄 수 있어야 합니다.
- 필터 반영 중에는 다른 필터 조작, 드릴다운, 조회 같은 액션을 잠시 막는 pending UX가 필요합니다.
- 상세 분석과 예시는 `docs/todo/ACTIVE_TODO.md`의 `DOC-012`에서 관리합니다.
