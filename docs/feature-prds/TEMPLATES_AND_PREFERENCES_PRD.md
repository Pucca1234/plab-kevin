...
- 저장 템플릿은 이름 변경, 삭제, 기본 템플릿 지정, 복제가 가능합니다.
- 사용자가 새 빈 탭을 만들면 저장 템플릿이 하나 생성되고, 탭 목록의 가장 오른쪽에 추가됩니다. 이 때 화면 상태도 빈 값으로 즉시 초기화됩니다.
- 사용자가 기존 템플릿을 복제하면 해당 템플릿의 모든 설정이 복사된 새 템플릿이 생성되며, 이름은 "[원본이름] 복사본"으로 자동 설정됩니다.
...
- 새 빈 탭 생성 시 가장 오른쪽 탭에 추가되고, 화면 상태가 즉시 초기화되는지 확인
- 템플릿 복제 시 원본과 동일한 설정으로 새 탭이 생성되는지 확인
- 복제된 탭 이름이 "[원본이름] 복사본"으로 설정되는지 확인
...
- 2026-06-24: 새 빈 탭 생성 시 화면 state 즉시 초기화(잔상 제거), 템플릿 복제 기능 추가
