...
- Next action:
  ...
  7. 회귀 시나리오:
     - 지표=`전체 매치 수`, 기간단위=`주`, 기간범위=`전체`, 측정단위=`전체`
     - 월 필터에서 `26.01`, `26.05`, `25.12`, `24.12` 선택
     - 이어서 분기 필터를 열면 `26.1`, `26.2`, `25.4`, `24.4`만 노출되어야 함
     - 같은 상태에서 월 필터를 다시 열어도 선택값이 `26.01` 하나로 축소되지 않고 기존 4개 선택이 유지되어야 함
- References:
  - `app/page.tsx`
  - `app/components/ControlBar.tsx`
  - `app/components/MultiSelectDropdown.tsx`
