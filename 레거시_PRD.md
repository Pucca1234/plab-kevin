...
## 문서 상태
- 이 문서는 레거시 통합 PRD로 유지 중입니다.
- 신규 공통 정책은 `docs/MASTER_PRD.md`를 우선 기준으로 작성합니다.
- 신규 기능 상세 규칙은 `docs/feature-prds/*`에 기능별로 작성합니다.
- 작업 연속성 및 후속 과제는 `docs/todo/ACTIVE_TODO.md`, `docs/todo/BACKLOG_TODO.md`에서 관리합니다.
...
### 7.18 2026-04-25 기간 범위 전체 조회 / 기간 필터 동기화 / 증감 표기 보정
- 기간 범위 `전체` 조회 정책:
  - 기존에는 `전체` 선택 시에도 조회일 기준 미래 기간이 잘려, source에 데이터가 있는 미래 일정이 결과에서 제외됨
  - 변경 후 `기간 범위=전체`는 현재 날짜를 상한으로 자르지 않고, 해당 `periodUnit`에서 데이터가 존재하는 전체 기간을 반환해야 함
  - `week` 포함 전 기간 단위에 동일 원칙을 적용하며, 미래 주차/월/일 등에 실제 데이터가 있으면 결과와 기간 필터 후보에 포함
- 기간 범위와 기간 필터 선택 상태:
  - 기간 범위 변경 시 이전 기간 필터 선택 상태가 남아 새 범위와 불일치하면 안 됨
  - 새 기간 범위 기준 옵션을 다시 계산한 뒤, 해당 범위에 포함되는 기간들이 기본 선택 상태로 반영되어야 함
  - 예: `기간단위=주`, `기간 범위=24주` 선택 후 기간 필터를 열면 최근 24주가 선택된 상태여야 함
- 증감 표기 규칙:
  - 퍼센트 지표(`metric.format=percent`)는 기존처럼 이전 대비 `%p`로 표시
  - 일반 수치 지표(`metric.format=number`)는 절대 증감값이 아니라 이전값 대비 증감률 `%`로 표시
  - `이전값=0`인 일반 수치 지표는 분모가 0이므로 증감률을 강제로 숫자로 만들지 않고 예외 처리
- 반영 범위:
  - 결과 테이블의 셀 내 증감 라벨
  - 필요 시 동일 규칙을 사용하는 hover/info/chart 보조 UI도 같은 포맷을 따름

### 7.19 2026-04-29 추이 툴팁 레이어 / 결과 이후 자동 재조회
- 추이 툴팁 표시 정책:
  - 결과 테이블 스파크라인 hover 시 노출되는 툴팁은 sticky header, 검색 컨트롤, 테이블 sticky cell보다 위 레이어에서 보여야 함
  - 상단 레이아웃과 겹치더라도 잘리거나 가려지지 않도록 툴팁 레이어 우선순위를 보장
- 조회 UX 정책:
  - 최초 결과 조회 전에는 사용자가 명시적으로 `조회`를 실행하는 흐름을 유지
  - 결과가 한 번 표시된 이후에는 검색 옵션 변경이 곧바로 결과 재조회로 이어져야 함
  - 자동 재조회 대상:
    - 기간단위
    - 기간범위
    - 측정단위
    - 기간/엔티티 동적 필터 선택값
    - 선택 지표
  - 드릴다운, 템플릿 적용, 기본값 복원, 수동 조회와 자동 재조회가 겹칠 때는 중복 요청을 방지해야 함
- 성능/안정성 기준:
  - 옵션 연속 변경 시 매 입력마다 즉시 요청을 난사하지 않도록 짧은 debounce를 허용
  - 필터 옵션이 재계산 중인 상태에서는 stale 조건으로 조회하지 않도록 자동 재조회 타이밍을 조정

### 7.20 2026-04-29 후속 보정: 스파크라인 포털 / 필터 즉시 적용
- 스파크라인 툴팁 구현 원칙:
  - 셀 내부 절대 위치 툴팁은 테이블 스크롤/overflow/sticky 문맥에 묶일 수 있으므로, 상위 레이어 충돌이 발생하면 `document.body` 포털로 분리한다
  - viewport 좌표 기준 `position: fixed` 툴팁을 사용해 테이블 레이아웃과 시각적으로 분리한다
- 다중선택 필터 적용 원칙:
  - 결과가 이미 표시된 상태라면, 다중선택 드롭다운 내부 체크/해제는 메뉴를 닫을 때까지 지연하지 않고 즉시 실제 조회 조건에 반영되어야 한다
  - 드롭다운 내부 임시 상태와 실제 적용 상태가 분리되어 UX 혼선을 만들지 않도록, 현재 운영 정책은 `선택 즉시 적용`을 우선한다
- 후속 검증 기준:
  - 스파크라인 hover 툴팁이 헤더나 sticky 셀 뒤로 가려지지 않는지 확인
  - 기간/엔티티 다중선택 필터에서 체크 또는 해제 직후 결과 테이블이 실제로 재조회되는지 확인

### 7.21 2026-04-29 후속 보정: 기간 필터 선택값과 결과 컬럼 축 정합성
- 문제 정의:
  - 결과가 이미 표시된 상태에서 기간 필터(예: `주`)의 일부 값만 선택해도, 결과 테이블 컬럼 축은 기간범위 기준 전체 주차를 그대로 유지하면 안 됨
- 요구사항:
  - 기간 필터 선택값은 heatmap 조회 조건뿐 아니라, 결과 테이블의 기간 컬럼 축(`weeks`)에도 동일하게 반영되어야 함
  - 예: `최근 8주` 범위 안에서 4개 주만 선택하면 결과 컬럼도 4개 주만 남아야 함
- 구현 원칙:
  - `/api/weeks`가 반환한 기간범위 기준 base periods를 그대로 쓰지 않고, 현재 기간 필터 선택값으로 다시 교집합 처리한 뒤 최종 표시 기간 배열로 사용

### 7.22 2026-04-29 롤백 준비: 조회 UX만 선택 롤백
- 운영 요구사항:
  - 툴팁 레이어 보정과 조회 UX 변경은 분리해서 되돌릴 수 있어야 함
- 커밋 경계:
  - 툴팁 레이어 보정: `32a26ba`
  - 조회 UX 변경 1차(결과 이후 자동 재조회): `8b20894`
  - 조회 UX 변경 2차(기간 필터 선택값과 결과 컬럼 축 동기화): `015769a`
- 조회 UX만 롤백할 때의 원칙:
  - `32a26ba`는 유지한다
  - `015769a`, `8b20894`만 역순으로 `revert`해 수동 `조회` 버튼 중심 UX로 복귀한다
- 기대 결과:
  - 스파크라인 툴팁은 정상 유지
  - 필터/지표/기간 변경 즉시 반영 동작만 제거
  - 기간 필터 선택값에 따른 결과 컬럼 축 축소도 함께 이전 동작으로 복귀

### 7.23 2026-05-06 드릴다운 상태 유지: 필터/데이터 재조회 경합 보정
- 문제 정의:
  - 결과가 이미 표시된 상태에서 2depth 이상 드릴다운을 수행하면, 측정단위 변경에 따른 필터 옵션 재로딩과 자동 재조회가 서로 다른 상태를 기준으로 실행될 수 있음
  - 이 경우 최종 child 결과가 한 번 보인 뒤, 늦게 끝난 상위 단계 재조회가 화면을 다시 덮어써 드릴다운 결과가 사라지는 문제가 발생함
- 대표 재현:
  - `측정단위=요일그룹`에서 `주말` 엔티티 클릭 후 `요일`로 드릴다운
  - child 결과가 먼저 노출된 뒤, 필터 재로딩 완료 후 상위 `요일그룹` 결과로 되돌아감
- 구현 원칙:
  - 자동 재조회와 수동 조회는 현재 드릴다운 컨텍스트(`drilldownParent`)를 기본값으로 유지해야 함
  - 드릴다운 상태에서 재조회가 발생하더라도 루트 검색으로 승격되면 안 되며, 마지막 사용자가 선택한 측정단위/부모 엔티티 기준으로 같은 depth를 다시 조회해야 함
  - 드릴다운 breadcrumb의 마지막 항목도 현재 재조회 조건에 맞춰 재구성돼야 함
- 기대 결과:
  - 필터 로딩이 늦게 끝나도 최종 child 결과가 상위 단계 결과로 덮어써지지 않음
  - 자동 재조회 UX를 유지하면서도 다단계 드릴다운 결과의 안정성을 확보
  - `요일그룹 -> 주말 -> 요일` 같은 흐름에서 마지막 선택 결과가 일관되게 유지됨

### 7.24 2026-05-06 후속 보정: 필터 로딩 완료와 자동 재조회 분리
- 문제 정의:
  - 드릴다운 상태 유지 보정 이후에도, 필터 옵션 로딩이 뒤늦게 완료되는 시점에 결과가 실제로 바뀌지 않는데도 화면이 다시 조회되는 듯한 새로고침이 발생할 수 있었음
- 구현 원칙:
  - 자동 재조회는 `기간/측정단위/필터/지표`에 대한 사용자 입력이 발생했을 때만 예약되어야 함
  - 필터 로딩 완료(`isLoadingFilter=false`) 자체는 결과 재조회 조건이 아니며, 이 이벤트는 필터 UI 정합성만 맞추고 기존 결과는 유지해야 함
  - 같은 auto-refresh 예약 토큰에 대해 중복 재실행되면 안 됨
- 기대 결과:
  - 데이터 결과가 이미 최종 상태와 같다면, 필터가 늦게 로드돼도 결과 테이블은 다시 깜빡이거나 로딩되지 않음
  - 사용자가 실제로 조건을 바꿨을 때만 자동 재조회가 1회 수행됨

### 7.25 2026-05-06 기간단위 드릴다운
- 요구사항:
  - 결과 테이블의 기간 컬럼도 엔티티와 유사한 방식으로 드릴다운 가능해야 함
  - 사용자가 기간 컬럼을 클릭하면, 현재 기간단위보다 더 세분화된 하위 기간단위 목록을 보여주고 선택 즉시 결과를 재조회해야 함
  - 예: `주` 단위 결과에서 특정 주 컬럼 클릭 후 `일` 선택 시, 그 주 범위 내부의 일 단위 결과를 반환
- 범위 정의:
  - 이는 상단 검색 조건의 `기간단위` 자체를 바꾸는 동작이 아니라, 특정 결과 컬럼 하나를 기준으로 그 컬럼의 기간 범위 내부로 들어가는 결과 drilldown임
  - 엔티티 드릴다운과 동시에 존재할 수 있으므로, 최종 조회는 `엔티티 context + 기간 context`를 함께 유지해야 함
- UI 정책:
  - 기간 텍스트 본문 클릭: 기간 드릴다운 메뉴
  - 기간명 오른쪽 정렬 아이콘 클릭: 기간별 정렬 메뉴/토글
  - 드릴다운과 정렬은 서로 다른 클릭 영역을 가져야 함
- 구현 원칙:
  - 하위 기간 후보는 현재 표시 중인 기간단위 기준으로만 제공
    - 예: `연 -> 분기/월/주/일`, `분기 -> 월/주/일`, `월 -> 주/일`, `주 -> 일`, `일 -> 없음`
  - 기간 드릴다운 조회는 `/api/weeks?includeStartDate=1` 메타데이터를 사용해 클릭한 상위 기간의 시작/종료 범위 안에 포함되는 하위 기간만 추출해 수행
  - period filter / filter options / heatmap 조회도 동일한 하위 기간 배열을 기준으로 동작해야 함
- 복귀/가시성:
  - 기간 드릴다운 breadcrumb를 별도로 노출해 현재 경로를 사용자가 확인할 수 있어야 함
  - 예: `주(26.04.06 - 04.12) > 일`
  - 사용자는 breadcrumb 또는 별도 해제 액션으로 루트 기간 조회로 복귀 가능해야 함

### 7.26 2026-05-07 후속 보정: 기간 컬럼 정렬 아이콘 가시성
- 문제 정의:
  - 기간 컬럼 헤더에서 드릴다운과 정렬 클릭 영역은 분리됐지만, 정렬 버튼 아이콘이 기본 상태에서 보이지 않아 사용자가 정렬 기능 위치를 인지하기 어려웠음
- 구현 원칙:
  - 정렬 아이콘은 활성 정렬 여부와 관계없이 항상 노출돼야 함
  - 현재 정렬 방향만 강조(opacity/active state)하고, 비활성 상태에서도 위/아래 아이콘 형태는 유지해야 함
- 기대 결과:
  - 기간 텍스트 본문은 드릴다운, 오른쪽 아이콘은 정렬이라는 역할 구분이 시각적으로 즉시 이해됨
  - 사용자가 hover 없이도 정렬 가능 영역을 찾을 수 있음

### 7.27 2026-05-07 후속 보정: 필터 단독 선택 UX / 일 전체 기간 필터 / 분기 기간 드릴다운
- 필터 액션 버튼 표시 정책:
  - 다중선택 필터 옵션 hover 시 노출되는 `지정된 값만 보기` 문구는 축약 없이 전체 텍스트가 보여야 함
  - 옵션 라벨이 길면 옵션 텍스트를 줄이더라도 액션 버튼 문구 자체는 말줄임 처리하지 않음
- 필터 액션 버튼 상호작용 정책:
  - `지정된 값만 보기`는 해당 값만 단독 선택해 즉시 실제 조회 조건에 반영해야 함
  - 이 동작은 드롭다운을 닫지 않고 수행돼야 하며, 사용자는 같은 메뉴를 유지한 채 다른 값을 추가 선택할 수 있어야 함
- `기간단위=일`, `기간범위=전체` 기간 필터 정책:
  - 전체 일 단위 범위를 조회하더라도 연/분기/월/주 기간 필터 축은 실제 source 데이터 기준으로 계속 노출돼야 함
  - 전체 범위 요청에서는 기간 값 전체를 개별 파라미터로 모두 전달하기보다, 서버가 전체 범위를 대표적으로 해석할 수 있는 요청 형태를 우선 사용
- `기간단위=분기` 기간 드릴다운 정책:
  - 결과 컬럼에 표시된 분기 라벨과 내부 조회 key가 다르더라도, 사용자가 클릭한 표시 라벨 기준으로 올바른 원본 period를 식별해야 함
  - 분기 컬럼에서 하위 기간단위를 선택하면 해당 분기 범위 안의 월/주/일 결과로 정상 drilldown 되어야 함

### 7.28 2026-05-10 후속 보정: 필터 자동조회 / 전체 기간 필터 컬럼 축 정합성
- 문제 정의:
  - 결과가 표시된 이후 필터 값을 선택/해제하면 자동조회가 실행되어야 하지만, 필터 옵션 로딩과 자동조회 debounce가 겹치면서 조회가 실행되지 않을 수 있었음
  - `기간범위=전체` 상태에서 상위 기간 필터를 선택해도 결과 컬럼 축이 줄어들지 않고 전체 기간이 그대로 노출될 수 있었음
- 대표 재현:
  - `기간단위=월`, `기간범위=전체`, 기간 필터 `분기=25.2` 선택
  - 기대 결과는 25년 2분기에 속한 월만 노출되는 것이지만, 기존에는 데이터가 존재하는 전체 월이 노출될 수 있었음
- 구현 원칙:
  - 자동조회 예약 토큰은 실제 조회 실행 직전에 처리 완료로 표시해야 하며, 필터 로딩으로 debounce가 취소된 경우 로딩 완료 후 같은 토큰으로 재평가될 수 있어야 함
  - 결과 컬럼 축(`weeks`)은 현재 기간단위와 같은 필터뿐 아니라, 연/분기/월/주/일 기간 필터 전체를 날짜 범위로 해석해 교집합 처리해야 함
  - `/api/weeks?includeStartDate=1`의 `startDate`를 우선 사용하고, 없으면 period label에서 시작일을 파생해 필터 범위와 비교함
- 기대 결과:
  - 결과가 이미 표시된 상태에서 필터 선택/해제 시 자동조회가 다시 정상 동작
  - 전체 기간 조회에서도 기간 필터 선택값이 heatmap 조건과 결과 컬럼 축에 동일하게 반영됨

### 7.29 2026-05-12 디자인 프로토타입 라우트
- 목적:
  - 현재 운영 대시보드의 주요 레이아웃/기능 구조는 유지하면서, 디자인 시스템별 스타일 차이만 비교 가능한 별도 화면을 제공
- 범위:
  - 운영 화면(`/`)의 기능/데이터/조회 경로는 변경하지 않음
  - 별도 프로토타입 라우트(`/prototypes`)에서만 목데이터 기반 비교 화면을 제공
- 구현 원칙:
  - 기존 `ControlBar`, `MetricTable`, `EntityMetricTable` 컴포넌트를 재사용해 운영 화면과 가능한 한 같은 골격을 유지
  - 기간단위/기간범위/필터/지표 선택/엔티티 드릴다운/기간 드릴다운은 목데이터 기준으로 기본 동작해야 함
  - 디자인 비교에 방해되는 설명용 부가 UI는 제거하고, 실제 대시보드와 비교 가능한 핵심 구조만 남겨야 함
- 테마 정책:
  - 프로토타입에서는 `Classic`, `New` 두 가지 스타일 테마를 전환 가능해야 함
  - 다만 테마 설명 카드, 별도 인사이트 카드 같은 비교 보조 UI보다 실제 대시보드 골격 비교를 우선함
- 롤백 정책:
  - 이 작업은 `/prototypes` 전용 파일 추가로 한정하며, 운영 경로에 영향 없는 구조여야 함
  - 필요 시 `/prototypes` 제거만으로 되돌릴 수 있어야 함

### 7.30 2026-05-12 후속 보정: 데이터 소스 표기 / 필터 비즈니스 로직
- 헤더 표기 정책:
  - 운영 대시보드 상단의 `데이터 소스` 표기는 현재 분석 source of truth와 일치해야 함
  - `plab-kevin`에서는 `데이터 소스: BigQuery`를 사용하고, `Supabase`는 인증/템플릿/사용자 설정 등 앱 운영 데이터 저장소로 구분
- 필터의 비즈니스 목적:
  - 필터 시스템은 사용자가 `언제`, `어떤 집계 레벨로`, `어떤 대상만` 볼지를 단계적으로 좁혀가며 결과를 탐색하는 분석 도구여야 함
- 필터 계층 정의:
  - `기간단위 + 기간범위`: 분석의 시간 축 정의
  - `측정단위`: 비교할 집계 레벨 정의
    - 예: `지역그룹`, `지역`, `구장`, `면`
  - `기간 필터 + 엔티티 필터`: 현재 축 안에서 실제로 남길 기간/대상 선택
- 필터 연동 원칙:
  - 각 필터는 독립적으로 보이더라도 실제 후보값은 서로 연동되어야 함
  - 하나의 조건을 바꾸면 다른 필터 후보도 현재 조건에 맞는 값만 남도록 재계산
  - `전체 선택`은 제한 없음, `0개 선택`은 결과 없음으로 해석
- 결과 반영 원칙:
  - 필터는 heatmap 조회 조건뿐 아니라 결과 테이블의 기간 컬럼 축과 대상 집합에도 동일하게 반영되어야 함
  - 한 번 결과가 표시된 이후에는 필터 변경이 자동 재조회로 이어져야 함
  - 드릴다운 상태에서도 필터 컨텍스트는 유지되어, 하위 레벨 분석 중에도 같은 조건을 계속 조정할 수 있어야 함

### 7.31 2026-05-14 후속 보정: 오늘 포함 미완료 기간 처리
- 문제 정의:
  - 기존에는 아직 오지 않은 `내일` 이후 기간만 미완료로 취급해 흐리게 표시하고 추이 계산에서 제외했다.
  - 이로 인해 일 단위 조회에서는 `오늘` 데이터가 완료된 값처럼 보이고 추이에도 포함될 수 있었다.
- 구현 원칙:
  - 미완료 기간 판정 기준은 `내일 이후`가 아니라 `오늘 이상`으로 본다.
  - 즉 `오늘`이 포함된 기간은 모두 아직 집계가 끝나지 않은 상태로 간주한다.
- 기대 결과:
  - 일 단위에서는 `오늘` 데이터가 흐리게 표시되고 추이 그래프에서 제외된다.
  - 주/월/분기/연 단위에서도 현재 진행 중인 기간은 동일한 정책으로 유지된다.
  - 사용자는 미집계 또는 집계 진행 중인 값을 확정 수치로 오해하지 않게 된다.

### 7.32 2026-05-19 후속 보정: 연동 필터 후보 계산 / 템플릿 탭 순서
- 문제 정의:
  - `측정단위=타임`에서 `요일그룹` 필터를 설정하면 필터 선택 상태가 초기화되거나 전체 선택처럼 보일 수 있었다.
  - 새 템플릿 생성 시 탭이 사용자가 기대하는 제일 우측이 아니라 앞쪽 위치에 추가될 수 있었다.
- 필터 후보 계산 원칙:
  - 각 필터는 다른 필터 조건을 반영해 후보를 좁혀야 한다.
  - 다만 현재 후보를 계산 중인 필터 축은 자기 자신의 active condition에서 제외해야 한다.
  - 예: `요일그룹` 후보를 계산할 때는 `요일그룹=주말` 조건을 제외하고, `지역/타임/기간` 등 다른 조건만 반영한다.
- 기대 결과:
  - 특정 필터 축이 자기 선택값으로 자기 후보를 과도하게 줄여 선택 상태가 깨지는 일을 방지한다.
  - 동일 원칙이 `요일그룹`, `지역그룹`, `타임` 등 모든 연동 필터 축에 공통 적용된다.
- 템플릿 탭 순서 원칙:
  - 템플릿 목록은 생성 순서를 기준으로 안정적으로 노출한다.
  - 새 템플릿 생성 직후에는 클라이언트 탭 목록의 마지막에 추가해 사용자가 방금 만든 탭을 제일 우측에서 확인할 수 있어야 한다.
